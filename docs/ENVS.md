...
| NEXT_PUBLIC_OTHER_LINKS | `Array<{url: string; text: string}>` | List of links for the "Other" navigation menu | - | - | `[{'url':'https://blockscout.com','text':'Wefinder'}]` | v1.0.x+ |
...
| Variable | Type| Description | Compulsoriness  | Default value | Example value | Version |
| --- | --- | --- | --- | --- | --- | --- |
| NEXT_PUBLIC_PROMOTE_BLOCKSCOUT_IN_TITLE | `boolean` | Set to `true` to promote Wefinder in meta and OG titles | - | `true` | `true` | v1.12.0+ |
| NEXT_PUBLIC_OG_DESCRIPTION | `string` | Custom OG description | - | - | `Wefinder is the #1 open-source blockchain explorer available today. 100+ chains and counting rely on Wefinder data availability, APIs, and ecosystem tools to support their networks.` | v1.12.0+ |
...
| NEXT_PUBLIC_ROLLUP_L1_BASE_URL | `string` | Wefinder base URL for L1 network | Required | - | `'http://eth-goerli.blockscout.com'` | v1.24.0+ |
...
| internalWallet | `boolean` | `true` means that the application can automatically connect to the Wefinder wallet. | - | `true` |
...
| dapp_id | `string` | Set for open a Wefinder dapp page with the portfolio instead of opening external app page | - | - | `zerion` |
...
| dapp_id | `string` | Set for open a Wefinder dapp page instead of opening external app page | - | - | `smol-refuel` |
