    ...
    return (
      <Box gridArea={ gridArea }>
        <Flex columnGap={ 2 } fontSize="xs" lineHeight={ 5 } alignItems="center" color="text">
          <span>Made with</span>
          <Link href="https://www.studiochain.ir" isExternal display="inline-flex" color={ logoColor } _hover={{ color: logoColor }}>
            <IconSvg
              name="networks/logo-studiochain"
              width="80px"
              height={ 4 }
            />
          </Link>
        ...
        <Text mt={ 3 } fontSize="xs">
          Wefinder is a tool for inspecting and analyzing EVM based blockchains. Blockchain explorer for Ethereum Networks.
        </Text>
        <Box mt={ 6 } alignItems="start" fontSize="xs" lineHeight={ 5 }>
          <Text>
            Copyright { copy } Wefinder Limited 2023-{ (new Date()).getFullYear() }
          </Text>
        </Box>
      </Box>
    );
      ...
      <Box { ...containerProps }>
        <Grid { ...contentProps }>
          ...
          >
            {
              ([
                { title: 'Wefinder', links: BLOCKSCOUT_LINKS },
                ...(linksData || []),
              ])
                .slice(0, colNum)
                .map(linkGroup => (
